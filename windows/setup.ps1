...
# --- 4. Install Custom WSL2 Kernel & Modules ---
$kernelScript = Join-Path -Path $PSScriptRoot -ChildPath "scripts\Install-WslKernel.ps1"
if (Test-Path -LiteralPath $kernelScript) {
    Write-Host "Checking custom WSL2 kernel..."
    & $kernelScript
} else {
    Write-Warning "Install-WslKernel script not found: $kernelScript"
}
...
# --- 6. Install Essential PowerShell Modules ---
